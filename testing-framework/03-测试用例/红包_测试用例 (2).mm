...
<map version="1.0.1">
<node TEXT="红包">
<node TEXT="TP-31 用户可以成功发送一个金额为1元的红包给单个好友">
...
<node TEXT="TP-32 用户可以成功发送一个金额为200元的红包给单个好友">
...
<node TEXT="TP-33 用户可以成功发送一个金额为0.01元的红包">
...
<node TEXT="TP-34 用户可以设置自定义金额发送红包">
...
<node TEXT="TP-35 用户可以发送带有祝福语的红包">
...
<node TEXT="TP-36 用户发送红包时，余额不足提示用户充值">
...
<node TEXT="TP-37 用户网络异常时发送红包失败并提示重试">
...
<node TEXT="TP-39 用户可以查看已发送红包的状态（已领取/未领取/已退回）">
...
<node TEXT="TP-40 用户接收到红包后，可以正常领取红包">
...
<node TEXT="TP-41 用户尝试领取重复的红包时系统提示已领取">
...
<node TEXT="TP-42 用户领取红包过程中网络异常后仍能查询红包领取状态">
...
<node TEXT="TP-43 用户24小时内未领取红包时，金额退回发起人余额">
...
<node TEXT="TP-44 红包金额小数点后超过两位时系统报错并提示输入有效金额">
...
<node TEXT="TP-45 红包金额为0或负数时无法发送，系统进行提示">
...
<node TEXT="TP-46 同一用户在短时间内发送多个红包不会触发风控限制">
...
<node TEXT="TP-47 同一用户在短时间内大量发送红包，系统触发风控并限制操作">
...
<node TEXT="TP-48 用户在微信好友聊天页面正常发送红包">
...
<node TEXT="TP-49 群聊中发送拼手气红包，系统随机分配金额并允许多用户领取">
...
<node TEXT="TP-50 红包发送和领取过程中用户信息和金额加密传输">
